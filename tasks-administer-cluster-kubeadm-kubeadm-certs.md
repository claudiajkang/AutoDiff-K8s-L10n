...
[쿠버네티스의 PKI 인증서 및 요구 조건](/ko/docs/setup/best-practices/certificates/)에 익숙해야 한다.

{{% /capture %}}

{{% capture steps %}}

## 맞춤 인증서 사용 {#custom-certificates}

기본적으로, kubeadm은 클러스터를 실행하는 데 필요한 모든 인증서를 생성한다.
자체 인증서를 제공하여 이 동작을 무시할 수 있다.

이렇게 하려면, `--cert-dir` 플래그 또는 kubeadm `ClusterConfiguration` 의
`certificatesDir` 필드에 지정된 디렉터리에 배치해야 한다.
기본적으로  `/etc/kubernetes/pki` 이다.

`kubeadm init` 을 실행하기 전에 지정된 인증서와 개인 키 쌍이 존재하면,
...
대신, controller-manager를 `--controllers=csrsigner` 와 함께
독립적으로 실행하고 인증서와 키를 가리킨다.

[PKI 인증서 및 요구 조건](/ko/docs/setup/best-practices/certificates/)은 외부 CA를
...
이 명령은 `/etc/kubernetes/pki` 에 저장된 CA(또는 프론트-프록시-CA(front-proxy-CA)) 인증서와 키를 사용하여 갱신을 수행한다.
...
CSR을 지정된 위치로 출력하기 위해 `--csr-dir` 디렉터리를 전달할 수 있다.
